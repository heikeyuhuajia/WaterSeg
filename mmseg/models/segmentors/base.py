...
class BaseSegmentor(BaseModule, metaclass=ABCMeta):
    ...
    def forward(self, img, img_metas, return_loss=True, **kwargs):
        ...
        if return_loss:
            ...
            return self.forward_test(img, img_metas, **kwargs)  # img: tensor.size([1, 3, 512, 512]), float32;
    ...
    def show_result(self,
        ...
        """
        img = mmcv.imread(img)  # loveda-img.shape(1024,1024,3)
        img = img.copy()
        seg = result[0]         # loveda- result[0].shape(1024,1024)
        if palette is None:
        ...
        palette = np.array(palette) # shape(7,3)
