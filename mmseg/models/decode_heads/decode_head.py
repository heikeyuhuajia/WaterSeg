...
class BaseDecodeHead(BaseModule, metaclass=ABCMeta):
    ...
    def __init__(self,
        ...
        self.channels = channels # 7.14-an3: self.channels = 512;
        self.num_classes = num_classes # 7.14-an4: 1;
        ...
        if isinstance(loss_decode, dict):
            self.loss_decode = build_loss(loss_decode) # 7.14-an5: 初始的loss字典 ['type':ce; ''; ''];
    ...
    def _init_inputs(self, in_channels, in_index, input_transform):
        ...
        self.input_transform = input_transform # 7.14-an2: multiple_select;
        self.in_index = in_index # 7.14-an1: in_index=[0,1,2,3];
        if input_transform is not None:
    ...
    def cls_seg(self, feat):
        ...
        output = self.conv_seg(feat)    # conv(1*1,out_channel=num_classes)
        return output

    def cls_seg_channels(self, feat):
        """Classify each pixel."""
        if self.dropout is not None:
            feat = self.dropout(feat)
        output = self.conv_seg_channels(feat)    # conv(1*1,out_channel=num_classes)
        return output
    ...
    def losses(self, seg_logit, seg_label):
        ...
        if self.sampler is not None:
            ...
            seg_weight = None
        seg_label = seg_label.squeeze(1)    #  [1, 1, 512, 512] -> [1, 512, 512]

        if not isinstance(self.loss_decode, nn.ModuleList):
            losses_decode = [self.loss_decode]  # losses_decode = CrossEntropy
        else:
            losses_decode = self.loss_decode
        for loss_decode in losses_decode:
            if loss_decode.loss_name not in loss:
                loss[loss_decode.loss_name] = loss_decode( # run
                    seg_logit,
                    seg_label,
                    weight=seg_weight,
